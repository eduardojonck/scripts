...
if [[ "$UBUNTU_VERSION" != "20.04" ]]; then
    log_error "Upgrade rejeitado: Este script suporta apenas UBUNTU 20.04."
    exit 1
fi
log_info "Versão do Ubuntu: $UBUNTU_VERSION"

rm -f /var/lib/dpkg/lock* /var/lib/apt/lists/lock /var/cache/apt/archives/lock
dpkg --configure -a

log_info "Habilitando acesso como root ao SSH...."
sed -i -e "s/^#*PermitRootLogin .*/PermitRootLogin yes/" /etc/ssh/sshd_config
sed -i -e "s/^#*DefaultTimeoutStopSec=.*/DefaultTimeoutStopSec=5s/" /etc/systemd/system.conf
systemctl restart ssh

log_info "Instalando PHP 7.2 para PNETLAB 6 do repositório de Eduardo Jonck <eduardo@eduardojonck.com>"
echo "deb [trusted=yes] https://eduardojonck.com/repo/pnetlab/v6/php7.2 focal main" > /etc/apt/sources.list.d/eduardojonck.list
apt-get update -qq
apt-get install -y \
php-common \
php7.2 \
...
if [ ! -e /opt/ovf/.configured ]; then
    ...
    log_info "Senha root configurada para padrão 'pnet'"
fi

resize_root_fs() {
    ROOTLV=$(mount | grep ' / ' | awk '{print $1}')
    if [[ "$ROOTLV" =~ ^/dev/mapper/ ]]; then
        log_info "Redimensionando volume lógico root ($ROOTLV)..."
        lvextend -l +100%FREE "$ROOTLV"
        resize2fs "$ROOTLV"
        log_info "Redimensionamento concluído."
    else
        log_warn "Volume root não é volume lógico. Pulando redimensionamento."
    fi
}
...
download_and_install() {
    ...
    if ! dpkg-query -l | grep -q "$pattern"; then
        if [ ! -f "/tmp/$filename" ]; then
            ...
            log_info "Arquivo $filename já existe. Pulando download."
        fi

        if [[ "$filename" =~ \.zip$ ]]; then
            log_info "Descompactando /tmp/$filename..."
            unzip -o "/tmp/$filename" -d "/tmp/${foldername:-unzip_folder}"
        elif [[ "$filename" =~ \.deb$ ]]; then
            log_info "Instalando pacote .deb /tmp/$filename..."
            dpkg -i "/tmp/$filename"
            return
        else
            log_warn "Formato do arquivo não suportado para instalação automática."
            return
        fi

        if [ -d "/tmp/$foldername" ]; then
            log_info "Instalando pacotes .deb em /tmp/$foldername..."
            dpkg -i /tmp/"$foldername"/"$foldername"/*.deb
        fi
    else
        log_info "Pacote com padrão '$pattern' já instalado. Pulando."
    fi
}

log_info "Baixando e instalando pacotes PNETLAB customizados..."
...
log_info "Instalando pacote principal pnetlab..."
...
if ! grep -q "pnetlab.example.com pnetlab" /etc/hosts; then
    ...
    log_info "Adicionado entrada no /etc/hosts"
fi
echo "pnetlab" >/etc/hostname

gcp_tune() {
    log_info "Detectado ambiente GCP. Ajustando configurações..."
    cd /sys/class/net/
    for i in ens*; do
...
azure_kernel_tune() {
    log_info "Detectado ambiente Azure. Ajustando configurações..."
    apt update
...
log_info "Limpeza de pacotes desnecessários..."
apt autoremove -y
apt autoclean -y

log_info "Upgrade finalizado com sucesso!"
log_info "Credenciais padrão: username=root password=pnet"

if [ ! -e /opt/ovf/.configured ]; then
    log_warn "Reinicie o sistema para aplicar alterações iniciais."
    read -rp "Deseja reiniciar agora? (s/N): " REBOOT_ANSWER
    if [[ "$REBOOT_ANSWER" =~ ^[Ss]$ ]]; then
        reboot
    else
        log_warn "Reinicie manualmente mais tarde."
    fi
fi
