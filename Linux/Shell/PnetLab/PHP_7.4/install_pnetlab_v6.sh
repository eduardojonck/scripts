...
if [ $? -ne 0 ]; then
    wget --content-disposition -q --show-progress $URL_KERNEL -O /tmp/$KERNEL
    unzip -o /tmp/$KERNEL -d /tmp/pnetlab_kernel
    dpkg -i /tmp/pnetlab_kernel/*.deb
fi

dpkg-query -l | grep docker-ce -q
if [ $? -ne 0 ]; then
    wget --content-disposition -q --show-progress $URL_PRE_DOCKER -O /tmp/pre-docker.zip
    unzip -o /tmp/pre-docker.zip -d /tmp/pre-docker
    dpkg -i /tmp/pre-docker/*.deb
fi

dpkg-query -l | grep swtpm -q
if [ $? -ne 0 ]; then
    wget --content-disposition -q --show-progress $URL_PNET_TPM -O /tmp/swtpm-focal.zip
    unzip -o /tmp/swtpm-focal.zip -d /tmp/swtpm-focal
    dpkg -i /tmp/swtpm-focal/*.deb
fi

dpkg-query -l | grep pnetlab-docker | grep 6.0.0-30 -q
if [ $? -ne 0 ]; then
    wget --content-disposition -q --show-progress $URL_PNET_DOCKER -O /tmp/pnetlab-docker.deb
    dpkg -i /tmp/pnetlab-docker.deb
fi

dpkg-query -l | grep pnetlab-schema | grep 6.0.0-30 -q
if [ $? -ne 0 ]; then
    wget --content-disposition -q --show-progress $URL_PNET_SCHEMA -O /tmp/pnetlab-schema.deb
    dpkg -i /tmp/pnetlab-schema.deb
fi

dpkg-query -l | grep pnetlab-guacamole | grep 6.0.0-7 -q
if [ $? -ne 0 ]; then
    wget --content-disposition -q --show-progress $URL_PNET_GUACAMOLE -O /tmp/pnetlab-guacamole.deb
    dpkg -i /tmp/pnetlab-guacamole.deb
fi

dpkg-query -l | grep pnetlab-vpcs | grep 6.0.0-30 -q
if [ $? -ne 0 ]; then
    wget --content-disposition -q --show-progress $URL_PNET_VPC -O /tmp/pnetlab-vpcs.deb
    dpkg -i /tmp/pnetlab-vpcs.deb
fi

dpkg-query -l | grep pnetlab-dynamips | grep 6.0.0-30 -q
if [ $? -ne 0 ]; then
    wget --content-disposition -q --show-progress $URL_PNET_DYNAMIPS -O /tmp/pnetlab-dynamips.deb
    dpkg -i /tmp/pnetlab-dynamips.deb
fi

dpkg-query -l | grep pnetlab-wireshark | grep 6.0.0-30 -q
if [ $? -ne 0 ]; then
    wget --content-disposition -q --show-progress $URL_PNET_WIRESHARK -O /tmp/pnetlab-wireshark.deb
    dpkg -i /tmp/pnetlab-wireshark.deb
fi

dpkg-query -l | grep pnetlab-qemu | grep 6.0.0-30 -q
if [ $? -ne 0 ]; then
    wget --content-disposition -q --show-progress $URL_PNET_QEMU -O /tmp/pnetlab-qemu.deb
    dpkg -i /tmp/pnetlab-qemu.deb
fi
...
wget --content-disposition -q --show-progress $URL_PNET_PNETLAB -O /tmp/pnetlab.deb
dpkg -i /tmp/pnetlab.deb
